...
# consider the fraction of the total area over a given width and
# create placeholder arrays to store this information
nel = max / dx + 1
...
for i in np.arange(nel):
    ...
    #print(width[i], f_frac[i], g_frac[i])
...
plt.plot(width, f_frac)
plt.plot(width, g_frac)
